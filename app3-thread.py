...
app.layout = html.Div([
    html.Div([
        ...
        html.Div([
            DataTable(id='table',
                      data=dfi.iloc[0:10][['series', 'q_val', 'curr', 'vol'
                                           ]].to_dict('records'),
                      columns=[{
                          "name": i,
                          "id": i
    ...
    html.Div([dcc.Graph(id='x1', figure=fig2)]),
...
@app.callback(Output('signal', 'data'), Input('req2', 'n_clicks'))
def put_and_alarm(value):
    return value


@app.callback(Output('c1', 'children'), Input('signal', 'data'))
def notified_and_render(value):
    return value


@app.callback(Output('c2', 'children'), Input('signal', 'data'))
def notified_and_render2(value):
    return value


@app.callback(Output('my-output2', 'children'), Input('req', 'n_clicks'))
...
@app.callback(Output(component_id='my-output', component_property='children'),
              Input(component_id='graph-with-slider',
